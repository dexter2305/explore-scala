...
class StringProblemsSuite extends munit.FunSuite:
  ...
  test("1108. Defanging IP address"):
    ...
    for (input, expected) <- testCases do
      assertEquals(StringProblems.defangingIPAddress(input), expected)
  ...
  test("709. To Lowercase"):
    ...
    for (input, e) <- testcases do
      assertEquals(StringProblems.toLowerCase(input), e)
  ...
  test("2129. Capitalize the title"):
    ...
    for (input, expected) <- testcases do
      assertEquals(StringProblems.capitalizeTheTitle(input), expected)
  ...
  test("520. Detect capital"):
    ...
    for (input, expected) <- testcases do
      assertEquals(StringProblems.detectCapital(input), expected)
  ...
  test("771. Jewels and stones"):
      ...
      for ((jewels, stones), expected) <- testcases do
          assertEquals(StringProblems.jewelsAndStones(jewels, stones), expected, clue = s"Jewels: '$jewels' && stones:'$stones' must return '$expected'")
  ...
  test("2011. Final value after performing operations"):
      ...
      for (input, expected) <- testcases do
        assertEquals(StringProblems.finalValueAfterOps(input), expected, clue = s"Operations: '${input.mkString(",")}' must return '$expected'")
  ...
  test("2942. Find words containing"):
      ...
      for ((words, x), expected) <- testcases do
        assert(StringProblems.findWordsContaining(words, x).sameElements(expected), clue = s"Expecting '${words.mkString(",")}' to contain '$x' in indices '${expected.mkString(",")}'")
  ...
  test("1662. Check if two strings are equivalent"):
      ...
      for((word1, word2), expected) <- testcases do
        assertEquals(StringProblems.arrayStringsAreEqual(word1, word2), expected, clue = s"Expecting equivalence between word1:'${word1.mkString(",")}' & word2:'${word2.mkString(",")}' as '$expected'")
  ...
  test("1816. Truncate sentence"):
      ...
      for (string, k, expected) <- testcases do
        assertEquals(StringProblems.truncateSentence(string, k), expected)
  ...
  test("1832. Check if sentence is pangram"):
    ...
    for (input, expected) <- testcases do
      assertEquals(StringProblems.checkIfPangram(input), expected)
  ...
  test("1859. Sorting the sequence"):
    ...
    for (input, expected) <- testcases do
      assertEquals(StringProblems.sortSentence(input), expected)
  ...
  test("2810. Faulty keyboard"):
    ...
    for (input, expected) <- testcases do
      assertEquals(StringProblems.finalString(input), expected)
  ...
  test("541. Reverse String II"):
    for (string, k, expected) <-
      List(
          ...
          assertEquals(StringProblems.reverseStr(string, k), expected)
  ...
  test("2828. Check if a string is an acronym of words"):
    for (words, s, expected) <- List(
      ...
      (List("never","gonna","give","up","on","you"), "ngguoy", true)
        ...
        assertEquals(StringProblems.isAcronym(words, s), expected)
  ...
  test("1684. Count the number of consistent strings"):
    for (words, allowed, expected) <- List(
      ...
      (Array("cc","acd","b","ba","bac","bad","ac","d"), "cad", 4)
        ...
        assertEquals(StringProblems.countConsistentStrings(allowed, words), expected, clue = s"Expecting '[${words.mkString(",")}]' to contain '${allowed}' '$expected' times.")
  ...
  test("917. Reverse only letters"):
    for (input, expected) <- List(
      ...
      ("Test1ng-Leet=code-Q!","Qedo1ct-eeLg=ntse-T!")
        ...
        assertEquals(StringProblems.reverseOnlyLetters(input), expected)
  ...
  test("1436. Destination city"):
      ...
      for (input, expected) <- testcases do
        assertEquals(StringProblems.destCity(input), expected)
  ...
  test("383. Can construct"):
      ...
      for (ransomeNote, magazine, expected) <- testcases do
        assertEquals(StringProblems.canConstruct(ransomeNote, magazine), expected, clue = s"Can construct $ransomeNote from $magazine should be $expected")
  ...
  test("2047. Number of valid words in a sentence"):
      ...
      for (testcase, expected) <- testcases do assertEquals(StringProblems.countValidWords(testcase), expected, clue = s"Expecting $testcase to return $expected")
