...
class StringProblemsFunsuite extends AnyFunSuite with ScalaCheckPropertyChecks:
  ...
  test("709. To Lowercase"):
    ...
    for (input, e) <- testcases do
      assert(StringProblems.toLowerCase(input) == e)
  ...
  test("771. Jewels and stones"):
      ...
      for ((jewels, stones), expected) <- testcases do
          assert(StringProblems.jewelsAndStones(jewels, stones) == expected, clue = s"Jewels: '$jewels' && stones:'$stones' must return '$expected'")
  ...
  test("2011. Final value after performing operations"):
      ...
      for (input, expected) <- testcases do
        assert(StringProblems.finalValueAfterOps(input) == expected, clue = s"Operations: '${input.mkString(",")}' must return '$expected'")
  ...
  test("1662. Check if two strings are equivalent"):
      ...
      for((word1, word2), expected) <- testcases do
        assert(StringProblems.arrayStringsAreEqual(word1, word2) == expected, clue = s"Expecting equivalence between word1:'${word1.mkString(",")}' & word2:'${word2.mkString(",")}' as '$expected'")
  ...
  test("541. Reverse String II"):
    for (string, k, expected) <-
      List(
          ...
          assert(StringProblems.reverseStr(string, k) == expected)
  ...
  test("2828. Check if a string is an acronym of words"):
    for (words, s, expected) <- List(
      ...
      (List("never","gonna","give","up","on","you"), "ngguoy", true)
        ...
        assert(StringProblems.isAcronym(words, s) == expected)
  ...
  test("1684. Count the number of consistent strings"):
    for (words, allowed, expected) <- List(
      ...
      (Array("cc","acd","b","ba","bac","bad","ac","d"), "cad", 4)
        ...
        assert(StringProblems.countConsistentStrings(allowed, words) == expected, clue = s"Expecting '[${words.mkString(",")}]' to contain '${allowed}' '$expected' times.")
  ...
  test("1436. Destination city"):
      ...
      for (input, expected) <- testcases do
        assert(StringProblems.destCity(input) == expected)
  ...
  test("383. Can construct"):
      ...
      for (ransomeNote, magazine, expected) <- testcases do
        assert(StringProblems.canConstruct(ransomeNote, magazine) == expected, clue = s"Can construct $ransomeNote from $magazine should be $expected")
  ...
  ignore("12047. Number of valid words in a sentence"):
      ...
      for (testcase, expected) <- testcases do assert(StringProblems.countValidWords(testcase) == expected, clue = s"Expected '${testcase}' to return '$expected'")

  test("2138. Divide a string into groups of size k"):
        val testcases = Table(
          ("s", "k", "fill", "expected"),
          ("abcdefghi", 3, 'x', Array("abc","def","ghi")),
          ("abcdefghij", 3, 'x', Array("abc","def","ghi","jx"))
        )
